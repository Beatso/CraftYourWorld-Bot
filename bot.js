...
const creationsWebhook = new Discord.WebhookClient(
	process.env.creationswebhookid,
	process.env.creationswebhooktoken
)
...
const scaleAndSend = (inputAttachment, channel) => {
	...
	request.get(inputAttachmentURL, (_err, _res, body) => {
		scaleImage(body)
			...
			.catch((error) => {
				channel.send(
					`There was an error trying to do that:\n\`${error}\``
				)
				// console.error(error)
			})
	})
...
client.on("message", (message) => {
	if (message.content === `rules`) {
		...
		const MembersRole = messageId.message.member.cache.find(
...
client.on("message", (message) => {
	...
	message
		.reply(
			"your message was deleted because it didn't have an attachment, image or link. Please use <#789838168308318219> for talking about creations posted in this channel."
		)
	...
	if (reaction.emoji.name == "⭐") {
		...
		if (
			...
			if (message.attachments.size != 0)
				embed.image = {
					url: message.attachments.entries().next().value[1]
						.attachment,
				}
			client.channels.cache
				.get("791635796431601715")
				.send({ embed: embed })
		}
	}
})
...
client.on("message", (message) => {
	if (
		...
		message
			.delete()
			.then((message) => message.reply("**Don't Send Invite Links!**"))
			.then((response) => response.delete({ timeout: 15000 }))
	}
})
...
client.on("message", (message) => {
	if (
		...
		message
			.delete()
			.then((message) =>
				message.reply("**Psst Don't Use Cursed Words!**")
			)
